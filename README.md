...
打开任意 `.mksk` 文件，应看到：段落标题与宏被高亮；编辑时 LSP 会给出类型错误、目标段落缺失、未声明变量等诊断（含宏参数类型/个数）。宏名补全会展示签名，hover 宏可查看其参数与说明，宏参数里会提示故事变量与段落名。跳转（`definitionProvider`）支持：链接 / `<<goto>>` / `<<display>>` → 目标段落；宏调用 → 其注册脚本（或 `<<widget>>` 定义处）；故事变量 → `vars.ts` 声明处。
